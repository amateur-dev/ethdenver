...
contract Manager is AccessControl, ReentrancyGuard {
    ////////// CHAINLINK VRF v1 /////////////////
    ...
    event SetWinnerTriggered(uint256 indexed raffleId, uint256 amountRaised, bytes32);
    ...
    function getRandomNumber(uint256 _id, uint256 _entriesSize, bytes32 _result)
        internal
        returns (bytes32 requestId)
    {
        ...
        chainlinkRaffleInfo[_result] = RaffleInfo({id: _id, size: _entriesSize});
        return _result;
    }
        ...
        external
    {
        ...
        console.log(raffleInfo.size);
        uint256 normalizedRandomNumber = (randomness % raffleInfo.size) + 1;
        console.log("normalizedRandomNumber", normalizedRandomNumber);
        ...
        console.log("calling TransferNFT");
    ...
    function temp_getResult(uint256 _raffleId) external returns (bytes32) {
        RaffleStruct storage raffle = raffles[_raffleId];
        ...
        bytes32 _result = keccak256(abi.encode(_raffleId));
        return _result;
    }
    ...
    //     // Check if the raffle is already accepted or is called again because early cashout failed
        ...
        //TODO: require that the timestamp has expired
    ...
    //     // this call trigers the VRF v1 process from Chainlink
        bytes32 _result = keccak256(abi.encode(_raffleId));
        getRandomNumber(_raffleId, raffle.entriesLength, _result);


        emit SetWinnerTriggered(_raffleId, raffle.amountRaised, _result);
        return _result;
    }
