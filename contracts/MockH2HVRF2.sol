    ...
    ConfirmedOwner
{
    ...
    struct RaffleStruct {
       ...
       // uint48 maxEntries; // maximum number of entries allowed per user, to avoid abuse
        uint48 totalEntriesCap; // total number of entries allowed. if 0 infinite entries
 ...
 /*   struct ClaimStruct {
    ...
    function fulfillRandomWords(
        ...
        transferFunds(s_requests[_requestId].id, normalizedRandomNumber);
    }
    ...
    function buyEntry(
        ...
        if (collectionWLLength > 0) {
            ...
            for (uint256 i; i < collectionWLLength; ++i) {
        ...
        onlyRole(OPERATOR_ROLE)
    {
        ...
        if (raffle.totalEntriesCap == raffle.entriesLength)
            ...
            uint256 normalizedRandomNumber = (_fakeRandomNumber[0] %
                raffle.entriesLength) + 1;
            closeRaffleAndRefund(_raffleId, normalizedRandomNumber);
        }

        emit SetWinnerTriggered(_raffleId, raffle.amountRaised);
    }

    function mockFulfillRandomness(
        uint256 raffleId,
        ...
        uint256 normalizedRandomNumber = (randomWords[0] %
            raffle.entriesLength) + 1;
