...
contract MockH2H is AccessControl, ReentrancyGuard, VRFConsumerBase {
    ////////// CHAINLINK VRF v1 /////////////////
    ...
    event Log(uint256 num1, uint256 num2);
    ...
    // Event sent when the ETH is staked it for the raffle
    ...
    function buyEntry(
        ...
        require(
            claimsData[hash].numEntriesPerUser + 1 <= raffle.maxEntries,
        ...
        //update claim data
        ++claimsData[hash].numEntriesPerUser;
...
*/
        ...
        onlyRole(OPERATOR_ROLE)
    {
        ...
        // only if the raffle is in accepted status the ETH staked is sent back to the operator
...
*/
    ...
    */
    ...
    function mockSetWinner(uint256 _raffleId, uint256 _fakeRandomNumber)
        external
        payable
        //  nonReentrant
        onlyRole(OPERATOR_ROLE)
    {
        ...
        emit Log(raffle.totalEntriesCap, raffle.entriesLength);

        if (raffle.totalEntriesCap == raffle.entriesLength)
            mockFulfillRandomness(_raffleId, _fakeRandomNumber);
        else closeRaffleAndRefund(_raffleId);

        emit SetWinnerTriggered(_raffleId, raffle.amountRaised);
    }

    function mockFulfillRandomness(uint256 raffleId, uint256 randomness)
        public
    {
        // randomness is the actual random number. Now extract from the aux map the original param id of the call
        RaffleStruct memory raffle = raffles[raffleId];
        // save the random number on the map with the original id as key
        uint256 normalizedRandomNumber = (randomness % raffle.entriesLength) +
            1;
        ...
        // send the event with the original id and the random number
        emit RandomNumberCreated(raffleId, randomness, normalizedRandomNumber);
    ...
    function createH2HRaffle(
        ...
        RaffleStruct memory raffle = RaffleStruct({
            ...
            maxEntries: 1, // in a H2H only 1 entry per user is allowed
    ...
    function closeRaffleAndRefund(uint256 _raffleId) internal {
        RaffleStruct storage raffle = raffles[_raffleId];
        ...
        // If nobody boughts, no need to refund anything. Just set status to cancelled
        if (raffle.entriesLength > 0) {
            uint256 amountToRefundPerPlayer = raffle.price;
            require(
                address(this).balance >
                    raffle.failureAmount +
                        (amountToRefundPerPlayer * (raffle.entriesLength - 1)),
                "Not enough balance"
            );

            emit Log(raffle.entriesLength, amountToRefundPerPlayer);
            for (uint256 i; i < raffle.entriesLength; ++i) {
                if (i == 0) // first player has special bonus
                {
                    (bool sentFundsBack, ) = entriesList[_raffleId][i]
                        .player
                        .call{value: raffle.failureAmount}("");
